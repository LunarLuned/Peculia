...
public class InvertedHealingTomeItem extends GenericTomeItem {
    public InvertedHealingTomeItem(Properties settings) {
        super(settings);
    }
        ...
        if (!level.isClientSide) {
            ...
            if (!player.isCrouching() && (player.getOffhandItem().getCount() >= 4) && player.getOffhandItem().is(ModItems.SOUL)) {
                ...
                for (LivingEntity livingEntity : nearbyEntities) {
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.HARM, 1, 1, true, true, true));
                }
                player.getOffhandItem().shrink(4);
                player.removeEffect(MobEffects.HARM);
                player.addEffect(new MobEffectInstance(MobEffects.HEAL, 1, 1, true, true, false));
