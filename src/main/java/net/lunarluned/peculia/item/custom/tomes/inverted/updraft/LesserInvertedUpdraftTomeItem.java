...
public class LesserInvertedUpdraftTomeItem extends GenericTomeItem {
    public LesserInvertedUpdraftTomeItem(Properties settings) {
        super(settings);
    }
        ...
        if (!level.isClientSide) {
            ...
            if (!player.isCrouching() && (player.getOffhandItem().getCount() >= 4) && player.getOffhandItem().is(ModItems.SOUL)) {
                ...
                for (LivingEntity livingEntity : nearbyEntities) {
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.LEVITATION, 10, 25, true, true, true));
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.UNLUCK, 200, 0, true, true, true));
                }
                player.getOffhandItem().shrink(4);
                player.removeEffect(MobEffects.LEVITATION);
                player.removeEffect(MobEffects.UNLUCK);
                player.addEffect(new MobEffectInstance(MobEffects.LEVITATION, 5, 29, true, true, false));
                player.addEffect(new MobEffectInstance(MobEffects.SLOW_FALLING, 30, 0, true, true, false));
                player.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 50, 0, true, true, false));
