...
public class LesserInvertedFortifyingTomeItem extends GenericTomeItem {
    public LesserInvertedFortifyingTomeItem(Properties settings) {
        super(settings);
    }
    ...
    public InteractionResultHolder<ItemStack> use(@NotNull Level level, @NotNull Player player, @NotNull InteractionHand interactionHand) {
        ...
        if (!level.isClientSide) {
            ...
            if (!player.isCrouching() && player.getOffhandItem().is(ModItems.SOUL)) {
                ...
                if (player.getOffhandItem().getCount() <= 47) {
            ...
            if (!player.isCrouching() && (player.getOffhandItem().getCount() >= 48) && player.getOffhandItem().is(ModItems.SOUL)) {

                for (LivingEntity livingEntity : nearbyEntities) {
                    livingEntity.addEffect(new MobEffectInstance(ModEffects.VOID_TOUCHED, 200, 0, true, true, true));
                }
                player.getOffhandItem().shrink(48);
                player.removeEffect(ModEffects.VOID_TOUCHED);
                player.addEffect(new MobEffectInstance(MobEffects.HEALTH_BOOST, 1100, 4, true, true, false));
