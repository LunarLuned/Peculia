...
public class InvertedAgilityTomeItem extends GenericTomeItem {
    public InvertedAgilityTomeItem(Properties settings) {
        super(settings);
    }
        ...
        if (!level.isClientSide) {
            ...
            if (!player.isCrouching() && (player.getOffhandItem().getCount() >= 4) && player.getOffhandItem().is(ModItems.SOUL)) {
                ...
                for (LivingEntity livingEntity : nearbyEntities) {
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 200, 0, true, true, true));
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.DIG_SLOWDOWN, 100, 0, true, true, true));
                }
                player.getOffhandItem().shrink(4);
                player.removeEffect(MobEffects.DIG_SLOWDOWN);
                player.removeEffect(MobEffects.MOVEMENT_SLOWDOWN);
                player.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 250, 1, true, true, false));
                player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 100, 0, true, true, false));
