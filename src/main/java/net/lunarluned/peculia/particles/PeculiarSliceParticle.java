...
public class PeculiarSliceParticle extends RisingParticle {

    public PeculiarSliceParticle(ClientLevel level, double x, double y, double z, double velX, double velY, double velZ) {
        super(level, x, y, z, velX, velY, velZ);
        this.scale(3.1F + (float) level.random.nextInt(6) / 10);
    ...
    public record Factory(SpriteSet sprites) implements ParticleProvider<SimpleParticleType> {
        public Particle createParticle(SimpleParticleType simpleParticleType, ClientLevel world, double x, double y, double z, double velX, double velY, double velZ) {
            PeculiarSliceParticle particle = new PeculiarSliceParticle(world, x, y, z, velX, velY, velZ);
