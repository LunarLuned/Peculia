        ...
        if (!user.level.isClientSide) {
            ...
            if (user.getOffhandItem().is(ModItems.SOUL)) {
                ...
                if (!user.isCrouching() && user.getOffhandItem().getCount() >= 4) {
                    ...
                    user.addEffect(new MobEffectInstance(MobEffects.LEVITATION, 5, 29, false, false, false));
                ...
                if (user.isCrouching() && user.getOffhandItem().getCount() >= 6) {
                    ...
                    spawnLesserUpdraftCloudAtPos(user, user.getOnPos(), 29);
                    spawnLesserUpdraftCloudTwoAtPos(user, user.getOnPos(), 29);
                    user.gameEvent(GameEvent.ITEM_INTERACT_START);
                    user.getOffhandItem().shrink(6);
                    user.addEffect(new MobEffectInstance(MobEffects.SLOW_FALLING, 60, 0));
